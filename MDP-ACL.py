...
from supervised import cut_mix_label_adaptive
...
def main():
    ...
    for epoch in range(epoch + 1, cfg['epochs']):
        ...
        for i, ((imgA_x, imgB_x, mask_x),
            ...
            pred_u_s2 = pred_u_s2.detach()
            conf_u_s2 = pred_u_s2.softmax(dim=1).max(dim=1)[0]
            mask_u_s2 = pred_u_s2.argmax(dim=1)

            entropy = -torch.sum(pred_u_s2 * torch.log(pred_u_s2 + 1e-10), dim=1)
            entropy /= np.log(cfg['nclass'])
            confidence = 1.0 - entropy
            confidence = confidence * conf_u_s2
            ...
            if np.random.uniform(0, 1) < trigger_prob and cfg["trainer"]["unsupervised"].get("use_cutmix", False):
                if cfg["trainer"]["unsupervised"].get("use_cutmix_adaptive", False):
                    imgB_s2_mix, mask_s2_mix, conf_s2_mix = cut_mix_label_adaptive(
                        imgB_u_s2,
                        mask_u_s2,
                        conf_u_s2,
                        imgB_x,
                    ...
                    imgA_s2_mix, maskA_s2_mix, confA_s2_mix = cut_mix_label_adaptive(
                        imgA_u_s2,
                        mask_u_s2,
                        conf_u_s2,
                        imgA_x,
            ...
            pred_s2_mix = model(imgA_s2_mix, imgB_s2_mix).detach()
            ...
            loss_s2_mix = criterion_u(pred_s2_mix, mask_s2_mix)
            loss_s2_mix = loss_s2_mix * (conf_s2_mix >= cfg['conf_thresh'])
            loss_s2_mix = loss_s2_mix.sum() / conf_s2_mix.numel()

            loss_u_w_fp = criterion_u(pred_u_w_fp, mask_u_w)
            loss_u_w_fp = loss_u_w_fp * ((conf_u_w >= cfg['conf_thresh']) & (ignore_mask != 255))
            loss_u_w_fp = loss_u_w_fp.sum() / (ignore_mask != 255).sum().item()

            loss = (loss_x + loss_u_s1 * 0.2 + loss_u_s2 * 0.2 + loss_u_w_fp * 0.4 + loss_s2_mix * 0.2) / 2.0
            ...
            total_loss_mix.update(loss_s2_mix.item())
            ...
            if rank == 0:
                ...
                writer.add_scalar('train/loss_s2_mix', loss_s2_mix.item(), iters)
                writer.add_scalar('train/mask_ratio', mask_ratio, iters)

            if (i % (len(trainloader_u) // 8) == 0) and (rank == 0):
                logger.info(
                    'Iters: {:}, Total loss: {:.3f}, Loss x: {:.3f}, Loss s: {:.3f}, Loss w_fp: {:.3f}, Loss mix: {:.3f}, Mask ratio: '
                    '{:.3f}'.format(i, total_loss.avg, total_loss_x.avg, total_loss_s.avg,
                                    total_loss_w_fp.avg, total_loss_mix.avg, total_mask_ratio.avg))
